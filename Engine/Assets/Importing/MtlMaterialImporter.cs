...
/// <summary>Imports common Wavefront MTL surface properties and diffuse texture references.</summary>
public sealed class MtlMaterialImporter : IAssetImporter
{
    ...
    public AssetImportResult Import(AssetDatabase database, AssetImportRequest request)
    {
        ...
        try
        {
            ...
            var materials = Parse(sourcePath);
            ...
            foreach (var parsed in materials)
            {
                ...
                if (!string.IsNullOrWhiteSpace(parsed.MapKd))
                {
                    var texturePath = ResolveTexturePath(sourcePath, parsed.MapKd);
                    if (File.Exists(texturePath))
                    {
                        var textureResult = new TextureAssetImporter().Import(database, new AssetImportRequest(texturePath, "Assets/Textures"));
                        if (textureResult.Success && textureResult.Assets.Count > 0)
                        {
                            textureGuid = textureResult.Assets[0].Guid;
                            imported.AddRange(textureResult.Assets);
                        }
                    }
                }
                ...
                var material = new MaterialAsset(guid, parsed.R, parsed.G, parsed.B, parsed.A, 0f, parsed.Roughness, textureGuid);
            ...
            return new(true, Id, imported, $"Imported {materials.Count} MTL material{(materials.Count == 1 ? "" : "s")} with referenced textures.");
        }
        catch (Exception ex)
        {
        ...
        foreach (var raw in File.ReadLines(path))
        {
            ...
            switch (parts[0])
            {
                ...
                case "map_Kd" when current != null && parts.Length >= 2:
                    current.MapKd = string.Join(" ", parts.Skip(1));
                    break;
            }
        }
        return result;
    ...
    private static string ResolveTexturePath(string mtlPath, string reference)
    {
        reference = reference.Trim().Trim('"');
        // Most Blender MTL files emit a plain relative filename. If exporter options are present,
        // the final token is the image path and is the safest fallback for this bootstrap importer.
        if (reference.Contains(' ')) reference = reference.Split(' ', StringSplitOptions.RemoveEmptyEntries).Last();
        return Path.GetFullPath(Path.Combine(Path.GetDirectoryName(mtlPath)!, reference));
    }
    ...
    private static string NormalizeDestination(string destination)
    {
        ...
        if (!normalized.Equals("Assets", StringComparison.OrdinalIgnoreCase) && !normalized.StartsWith("Assets/", StringComparison.OrdinalIgnoreCase)) normalized = "Assets/" + normalized;
        return normalized;
    }
