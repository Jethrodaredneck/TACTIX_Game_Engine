...
public sealed class MetalRenderer : IDisposable
{
    ...
    private readonly Dictionary<AssetGuid, TextureGpu> _textures = new();
    ...
    private readonly record struct SurfaceData(Vector4 BaseColor, IMTLTexture? Texture);
    ...
    {
        AddMesh(BuiltInMesh.Cube, Cube()); AddMesh(BuiltInMesh.Plane, Plane()); AddMesh(BuiltInMesh.Sphere, Sphere(24, 16));
        AddMesh(BuiltInMesh.Cylinder, Cylinder(28)); AddMesh(BuiltInMesh.Cone, Cone(28)); AddMesh(BuiltInMesh.Capsule, Capsule(24, 16));
    }
    ...
    private static void V(List<float> a, Vector3 p, Vector3 n, float u, float v) { a.Add(p.X); a.Add(p.Y); a.Add(p.Z); a.Add(n.X); a.Add(n.Y); a.Add(n.Z); a.Add(u); a.Add(v); }
    private static void Quad(List<float> a, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 n) { V(a,p0,n,0,1); V(a,p1,n,1,0); V(a,p2,n,1,1); V(a,p0,n,0,1); V(a,p3,n,0,0); V(a,p1,n,1,0); }
    ...
    {
        var a=new List<float>(); Vector3 P(float u,float v){var theta=u*MathF.PI*2;var phi=(v-.5f)*MathF.PI;var cp=MathF.Cos(phi);return new(cp*MathF.Sin(theta),MathF.Sin(phi),cp*MathF.Cos(theta));}
        for(var y=0;y<stacks;y++) for(var x=0;x<slices;x++){var u0=(float)x/slices;var u1=(float)(x+1)/slices;var v0=(float)y/stacks;var v1=(float)(y+1)/stacks;var p00=P(u0,v0);var p10=P(u1,v0);var p01=P(u0,v1);var p11=P(u1,v1);V(a,p00,p00,u0,v0);V(a,p11,p11,u1,v1);V(a,p10,p10,u1,v0);V(a,p00,p00,u0,v0);V(a,p01,p01,u0,v1);V(a,p11,p11,u1,v1);} return a.ToArray();
    }

    private static float[] Cylinder(int segments)
    {
        var a=new List<float>(); for(var i=0;i<segments;i++){var u0=(float)i/segments;var u1=(float)(i+1)/segments;var t0=u0*MathF.PI*2;var t1=u1*MathF.PI*2;var n0=new Vector3(MathF.Sin(t0),0,MathF.Cos(t0));var n1=new Vector3(MathF.Sin(t1),0,MathF.Cos(t1));var b0=new Vector3(n0.X,-1,n0.Z);var b1=new Vector3(n1.X,-1,n1.Z);var t0p=new Vector3(n0.X,1,n0.Z);var t1p=new Vector3(n1.X,1,n1.Z);V(a,b0,n0,u0,1);V(a,t1p,n1,u1,0);V(a,b1,n1,u1,1);V(a,b0,n0,u0,1);V(a,t0p,n0,u0,0);V(a,t1p,n1,u1,0);V(a,Vector3.UnitY,Vector3.UnitY,.5f,.5f);V(a,t1p,Vector3.UnitY,1,1);V(a,t0p,Vector3.UnitY,0,1);V(a,-Vector3.UnitY,-Vector3.UnitY,.5f,.5f);V(a,b0,-Vector3.UnitY,0,1);V(a,b1,-Vector3.UnitY,1,1);} return a.ToArray();
    }

    private static float[] Cone(int segments)
    {
        var a=new List<float>();var apex=new Vector3(0,1,0);for(var i=0;i<segments;i++){var u0=(float)i/segments;var u1=(float)(i+1)/segments;var t0=u0*MathF.PI*2;var t1=u1*MathF.PI*2;var b0=new Vector3(MathF.Sin(t0),-1,MathF.Cos(t0));var b1=new Vector3(MathF.Sin(t1),-1,MathF.Cos(t1));var sideNormal=Vector3.Normalize(Vector3.Cross(b1-b0,apex-b0));V(a,apex,sideNormal,.5f,0);V(a,b1,sideNormal,u1,1);V(a,b0,sideNormal,u0,1);V(a,-Vector3.UnitY,-Vector3.UnitY,.5f,.5f);V(a,b0,-Vector3.UnitY,0,1);V(a,b1,-Vector3.UnitY,1,1);}return a.ToArray();
    }

    private static float[] Capsule(int slices,int stacks)
    {
        var a=new List<float>();Vector3 P(float u,float v,out Vector3 normal){var theta=u*MathF.PI*2;var phi=(v-.5f)*MathF.PI;var cp=MathF.Cos(phi);normal=new Vector3(cp*MathF.Sin(theta),MathF.Sin(phi),cp*MathF.Cos(theta));var offset=v>.5f?.65f:-.65f;return new Vector3(normal.X,normal.Y+offset,normal.Z);}for(var y=0;y<stacks;y++)for(var x=0;x<slices;x++){var u0=(float)x/slices;var u1=(float)(x+1)/slices;var v0=(float)y/stacks;var v1=(float)(y+1)/stacks;var p00=P(u0,v0,out var n00);var p10=P(u1,v0,out var n10);var p01=P(u0,v1,out var n01);var p11=P(u1,v1,out var n11);V(a,p00,n00,u0,v0);V(a,p11,n11,u1,v1);V(a,p10,n10,u1,v0);V(a,p00,n00,u0,v0);V(a,p01,n01,u0,v1);V(a,p11,n11,u1,v1);}return a.ToArray();
    }
    ...
    {
        using var data=NSData.FromFile(path)??throw new InvalidOperationException("Failed to read texture data.");
        using var source=CGImageSource.FromData(data)??throw new InvalidOperationException("Failed to decode texture.");
        using var image=source.CreateImage(0,null)??throw new InvalidOperationException("Failed to create CGImage.");
        var w=(int)image.Width;var h=(int)image.Height;var bytes=new byte[w*h*4];var handle=GCHandle.Alloc(bytes,GCHandleType.Pinned);
        try{using var cs=CGColorSpace.CreateDeviceRGB();using(var ctx=new CGBitmapContext(handle.AddrOfPinnedObject(),w,h,8,w*4,cs,CGBitmapFlags.ByteOrder32Big|CGBitmapFlags.PremultipliedLast)){ctx.TranslateCTM(0,h);ctx.ScaleCTM(1,-1);ctx.DrawImage(new CGRect(0,0,w,h),image);}var td=MTLTextureDescriptor.CreateTexture2DDescriptor(MTLPixelFormat.RGBA8Unorm,(nuint)w,(nuint)h,false);td.Usage=MTLTextureUsage.ShaderRead;var tex=_device.CreateTexture(td);tex.ReplaceRegion(new MTLRegion(new MTLOrigin(0,0,0),new MTLSize((nint)w,(nint)h,1)),0,handle.AddrOfPinnedObject(),(nuint)(w*4));return tex;}finally{handle.Free();}
    }

    private DirectionalLightData ResolveDirectionalLight()
    {
        if(_world!=null){foreach(var(entity,light)in _world.Query<LightComponent>()){if(!light.Enabled||light.Type!=LightType.Directional)continue;var rotation=_world.Has<TransformComponent>(entity)?_world.Get<TransformComponent>(entity).Rotation:new Vector3(50,-30,0);return new DirectionalLightData(RotateEuler(Vector3.UnitZ,rotation),light.Color,MathF.Max(0,light.Intensity));}}
        return new DirectionalLightData(Vector3.Normalize(new Vector3(.4f,-1f,.3f)),new Vector3(1f,.96f,.88f),1.15f);
    }

    private LocalLightData ResolveLocalLight()
    {
        if(_world!=null){foreach(var(entity,light)in _world.Query<LightComponent>()){if(!light.Enabled||light.Type==LightType.Directional)continue;var transform=_world.Has<TransformComponent>(entity)?_world.Get<TransformComponent>(entity):TransformComponent.Identity;var type=light.Type==LightType.Point?1f:2f;var direction=RotateEuler(Vector3.UnitZ,transform.Rotation);var inner=MathF.Cos(Math.Clamp(light.InnerConeDegrees,0f,89.9f)*MathF.PI/180f);var outer=MathF.Cos(Math.Clamp(MathF.Max(light.OuterConeDegrees,light.InnerConeDegrees+.01f),.01f,89.99f)*MathF.PI/180f);return new LocalLightData(type,transform.Position,direction,light.Color,MathF.Max(0,light.Intensity),MathF.Max(.01f,light.Range),inner,outer);}}
    ...
    {
        var r=MathF.PI/180f;var x=degrees.X*r;var y=degrees.Y*r;var z=degrees.Z*r;var sx=MathF.Sin(x);var cx=MathF.Cos(x);var sy=MathF.Sin(y);var cy=MathF.Cos(y);var sz=MathF.Sin(z);var cz=MathF.Cos(z);vector=new Vector3(vector.X,cx*vector.Y-sx*vector.Z,sx*vector.Y+cx*vector.Z);vector=new Vector3(cy*vector.X+sy*vector.Z,vector.Y,-sy*vector.X+cy*vector.Z);vector=new Vector3(cz*vector.X-sz*vector.Y,sz*vector.X+cz*vector.Y,vector.Z);return Vector3.Normalize(vector);
    }

    private TerrainGpu? GetTerrainGpu(AssetGuid guid)
    {
        if(_assets==null||!_assets.Registry.TryGet(guid,out var meta))return null;if(_terrainMeshes.TryGetValue(guid,out var cached)&&cached.ContentHash==meta.ContentHash)return cached;if(_terrainMeshes.Remove(guid,out var old))old.Buffer.Dispose();var terrain=_assets.LoadTerrain(guid);var mesh=TerrainMeshGenerator.Generate(terrain);var expanded=new float[mesh.Indices.Length*8];var dst=0;foreach(var index in mesh.Indices){var i=checked((int)index);var p=i*3;var uv=i*2;expanded[dst++]=mesh.Positions[p];expanded[dst++]=mesh.Positions[p+1];expanded[dst++]=mesh.Positions[p+2];expanded[dst++]=mesh.Normals[p];expanded[dst++]=mesh.Normals[p+1];expanded[dst++]=mesh.Normals[p+2];expanded[dst++]=mesh.UV0[uv];expanded[dst++]=mesh.UV0[uv+1];}var handle=GCHandle.Alloc(expanded,GCHandleType.Pinned);try{var buffer=_device.CreateBuffer(handle.AddrOfPinnedObject(),(nuint)(expanded.Length*sizeof(float)),MTLResourceOptions.CpuCacheModeDefault);var gpu=new TerrainGpu{Buffer=buffer,VertexCount=expanded.Length/8,ContentHash=meta.ContentHash};_terrainMeshes[guid]=gpu;return gpu;}finally{handle.Free();}
    }
    ...
    {
        if(_assets==null||!_assets.Registry.TryGet(guid,out var meta)||meta.Type!=AssetType.Texture)return null;
        if(_textures.TryGetValue(guid,out var cached)&&cached.ContentHash==meta.ContentHash)return cached.Texture;
        if(_textures.Remove(guid,out var old))old.Texture.Dispose();
    ...
    public void Dispose()
    {
        foreach(var mesh in _meshes.Values)mesh.Buffer.Dispose();_meshes.Clear();foreach(var terrain in _terrainMeshes.Values)terrain.Buffer.Dispose();_terrainMeshes.Clear();foreach(var mesh in _assetMeshes.Values)mesh.Buffer.Dispose();_assetMeshes.Clear();foreach(var texture in _textures.Values)texture.Texture.Dispose();_textures.Clear();_logoTexture.Dispose();_sampler.Dispose();_depthState.Dispose();_pipeline.Dispose();_library.Dispose();_queue.Dispose();
    }
}
