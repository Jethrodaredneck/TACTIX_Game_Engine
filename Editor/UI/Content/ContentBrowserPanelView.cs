...
public sealed class ContentBrowserPanelView : NSView
{
    ...
    private readonly NSButton _importButton;
    private readonly NSButton _pendingButton;
    ...
    public ContentBrowserPanelView(CGRect frame, AssetDatabase assets) : base(frame)
    {
        ...
        _importButton = MakeToolbarButton("Import...", ImportFromFileDialog, accent: true);
        _pendingButton = MakeToolbarButton("Import Pending", ImportPending);
        AddSubview(_importButton);
        AddSubview(_pendingButton);
    ...
    public override void Layout()
    {
        ...
        _importButton.Frame = new CGRect(margin, importY, 82, 24);
        _pendingButton.Frame = new CGRect(margin + 88, importY, 116, 24);
    ...
    private static NSButton MakeToolbarButton(string title, Action action, bool accent = false)
    {
        var button = new NSButton(new CGRect(0, 0, 80, 24)) { Title = title };
    ...
    private static string BadgeForFile(string file, string extension)
    {
        if (extension == ".tasset")
        {
            return TryReadAssetType(file, out var type)
                ? type switch
                {
    ...
    private static bool TryReadAssetType(string file, out AssetType type)
    {
        try
        {
            type = JsonAssetSerializer.Load(file).Meta.Type;
            return true;
        }
        catch
        {
    ...
    private Action? ReimportActionFor(string file, string extension)
    {
        ...
        try
        {
            ...
            if (assetFile.Meta.Type != AssetType.Model || string.IsNullOrWhiteSpace(assetFile.Meta.SourcePath))
                return null;
    ...
    private void ImportFromFileDialog()
    {
        ...
        panel.Title = "Import TACTIX Assets";
        panel.CanChooseFiles = true;
        panel.CanChooseDirectories = false;
        panel.AllowsMultipleSelection = true;

        if (panel.RunModal() != (nint)NSModalResponse.OK)
            return;
        ...
        foreach (var url in panel.Urls)
        {
            if (!string.IsNullOrWhiteSpace(url.Path))
                results.Add(ImportPath(url.Path));
        }
    ...
    private void ImportPending()
    {
        try
        {
            ...
            if (results.Count == 0)
            {
                _status.StringValue = "No pending imports in Imports";
                return;
            }
        ...
        {
            _status.StringValue = "Import pending failed: " + ex.Message;
        }
    }
    ...
    private void ReimportAsset(string assetFilePath)
    {
        try
        {
            ...
            {
                _status.StringValue = "Reimport source missing: " + assetFile.Meta.SourcePath;
                return;
            }

            var settings = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ["assetProjectPath"] = assetFile.Meta.ProjectPath
            };
            ReportResults([_importPipeline.Import(_assets, new AssetImportRequest(sourcePath, "Assets/Models", settings))]);
        }
        catch (Exception ex)
        {
            _status.StringValue = "Reimport failed: " + ex.Message;
        }
    }

    private AssetImportResult ImportPath(string sourcePath)
        => _importPipeline.Import(_assets, new AssetImportRequest(sourcePath, "Assets/Models"));
    ...
    private void ReportResults(IReadOnlyList<AssetImportResult> results)
    {
        ...
        {
            _status.StringValue = failures[0].Message;
            return;
        }
        ...
        _status.StringValue = importedCount switch
        {
            ...
            1 => results.First(r => r.Assets.Count > 0).Message,
            _ => $"Imported {importedCount} assets"
        };
    }
    ...
    private void RevealLastImported(IReadOnlyList<AssetImportResult> results)
    {
        ...
        _relativeFolder = folder.StartsWith("Assets/", StringComparison.OrdinalIgnoreCase)
