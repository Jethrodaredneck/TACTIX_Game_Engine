...
public sealed class InspectorPanelView : NSView
{
    ...
    public override void Layout()
    {
        ...
        _componentCard.Frame = new CGRect(10, Math.Max(10, Bounds.Height - 390), width, 176);
        _componentTitle.Frame = new CGRect(12, 144, width - 24, 20);
        _componentSummary.Frame = new CGRect(12, 124, width - 24, 17);
        for (var i = 0; i < 4; i++)
        {
            var y = 94 - i * 29;
    ...
    private void Refresh()
    {
        ...
        else if (_world.Has<MeshRendererComponent>(entity))
        {
            ...
            _componentSummary.StringValue = $"Mesh  {mesh.Mesh}   ·   Material  {mesh.Material}";
        }
        else
        {
            _componentCard.Hidden = true;
    ...
    private void ApplyTransform(int index)
    {
        ...
        if (index < 3)
        {
            var v = after.Position; if (index == 0) v.X = value; else if (index == 1) v.Y = value; else v.Z = value; after.Position = v;
        }
        else if (index < 6)
        {
            var v = after.Rotation; if (index == 3) v.X = value; else if (index == 4) v.Y = value; else v.Z = value; after.Rotation = v;
        }
        else
        {
            var v = after.Scale; if (index == 6) v.X = value; else if (index == 7) v.Y = value; else v.Z = value; after.Scale = v;
        }
